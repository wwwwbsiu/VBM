...
#-------------------------------2024/7/16---------------------------------------------#
    def replace_batchnorm(self,net):
        for child_name, child in net.named_children():
            if hasattr(child, 'fuse'):
                fused = child.fuse()
                setattr(net, child_name, fused)
                self.replace_batchnorm(fused)
...
#-----------------------------------end---------------------------------------------#
    ...
    def fuse(self, verbose=True):
        """
        Fuse the `Conv2d()` 、 `BatchNorm2d()` and `C2f_Rep` layers of the model into a single layer, in order to improve the
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        #---------------------------START------------------------------------#
        elif m in [SE,SaELayer]:
            args=[ch[f],*args]
            c2=ch[f]

        # elif m is EMA: # EMA模块以默认通道数 8 为组
        #     args=[ch[f],*args]
        #     c2=ch[f]   

        elif m is EMA: # EMA模块以通道数为分组数
            args=[ch[f],ch[f]]
            c2=ch[f]            
