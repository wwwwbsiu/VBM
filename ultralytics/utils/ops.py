...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if rotated:
            ...
            #-----------------------------------------------------------------------------#
            #  NMS改进
            # i = NMS(boxes, scores, iou_thres,Shape_IoU=True)
...
#-----------------------------------改进NMS------------------------------------------#
# ！！！！！！！！！！！！！  1.改
def NMS(boxes, scores, iou_thres, GIoU=False, DIoU=False, CIoU=False,EIoU=False,MDPIoU=False,Shape_IoU=False):
    #-----------------------------------------#
    #  获取scores从大到小的排列序号,放入B中
    #-----------------------------------------#
    B = torch.argsort(scores, dim=-1, descending=True)
    keep = []
    while B.numel() > 0:
        #-------------------------------------------#
        #  记录并将置信度最大的框的下标保存到keep中
        #-------------------------------------------#
        index = B[0]
        keep.append(index)
        if B.numel() == 1: break
        #--------------------------------------------------------#
        #  计算最大置信度框与剩下的框的iou(不包含最大置信度框)
        #  将iou低于阈值的框的序号放入B中
        #--------------------------------------------------------#
        # ！！！！！！！！！！！！！  2.改
        iou=bbox_iou(boxes[index, :], boxes[B[1:], :],xywh=False,GIoU=GIoU, DIoU=DIoU, CIoU=CIoU,EIoU=EIoU,MDPIoU=MDPIoU,Shape_IoU=Shape_IoU)
        iou=iou.squeeze()
...
def soft_nms(bboxes, scores, iou_thresh=0.5,sigma=0.5,score_threshold=0.25):
    ...
    while order.numel() > 0:
        ...
        #----------------------------------------------------------#
        #  获取 按order排序的分数 中满足阈值的box的下标(不包括刚刚放入keep的下标)
        #----------------------------------------------------------#
