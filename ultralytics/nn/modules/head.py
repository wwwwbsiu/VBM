...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        #-------------------------------------------------------------------#
        #  box :torch.Size([1, 64, 8400])  64: 4x16
        #  dbox:torch.Size([1, 4, 8400])   解码 将离散的值转换为 原图的 xywh 
        #-------------------------------------------------------------------#
...
#         c2, c3 = max((ch[0] // 4, self.reg_max * 4, 16)), max(ch[0], min((self.nc, 100)))  # channels
#         c4, c5 = max((ch[self.nl] // 4, self.reg_max * 4, 16)), max(ch[self.nl], min((self.nc, 100)))  # channels
#         self.cv2 = nn.ModuleList(
