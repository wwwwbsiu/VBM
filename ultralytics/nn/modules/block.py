...
# ---1.渐进架构部分（融合前的准备）--- #
    def __init__(self, c1, c2, scale_factor=2):
        ...
        if scale_factor == 2:
            ...
            self.cv1 = nn.ConvTranspose2d(c1, c2, 8, 8, 0, bias=True)  # 如果下采样率为8，就用Stride为8的8×8卷积来实现4次下采样
    def forward(self, x):
...
class NewDANetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer=nn.BatchNorm2d):
        super(NewDANetHead, self).__init__()
...
class New_Dual_DANetHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer=nn.BatchNorm2d):
        super(New_Dual_DANetHead, self).__init__()
        inter_channels = in_channels // 2
        self.conv5a = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 3, padding=1, bias=False),
                                   norm_layer(inter_channels),
                                   nn.ReLU())
